...
def AM(params, ns):
    """
    Model with ancient migration

    nu1: pop size for pop 1
    nu2: pop size for pop 2 
    T1: time of split
    T2: time at which migration stops
...
for i in range(1,6):
    ...
    Optimize_Functions.Optimize_Routine(data, prefix, "AM", AM, 3, 6, data_folded=True, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
