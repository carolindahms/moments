...
def SC_G(params, ns):
    """
    Secondary contact model with exponential pop growth 

    T1: Time in the past of split (in units of 2*Na generations) 
    nu10: Pop 1 size at time of split
    nu20: Pop 2 size at time of split
    T2: Time of secondary contact, migration begins
    nu1: Pop 1 size at time of secondary contact
    nu2: Pop 2 size at time of secondary contact
    ...
    """
    nu10,nu20,nu1,nu2,nu1b,nu2b,T1,T2,m12,m21 = params
    ...
    fs.integrate(nub_func, T2, dt_fac=0.01, m=numpy.array([[0,m12],[m21,0]]))
    return fs

p_labels = "nu10,nu20,nu1,nu2,nu1b,nu2b,T1,T2,m12,m21"
upper = [20,20,20,20,20,20,10,10,200,200]
...
for i in range(1,6):
    ...
    Optimize_Functions.Optimize_Routine(data, prefix, "SC_G", SC_G, 5, 10, data_folded=True, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
