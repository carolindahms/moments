...
def SI_NeC(params, ns):
    """
    Strict isolation model with instantaneous Ne changes
...
lower = [1e-3,1e-3,1e-3,1e-3,1e-3,1e-2]
...
for i in range(1,6):
    ...
    Optimize_Functions.Optimize_Routine(data, prefix, "SI_NeC", SI_NeC, 4, 6, data_folded=True, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)